...
# 支持多个时间间隔数据集的训练与评估
# 移除了SG滤波器、小波变换(SWT)和对比学习模块，其他模块保持不变
# 移除了所有绘图功能，仅保留数据文件
...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\Ablation\\CPU\\SWT-CLSTM-SSC\\cpu_results_Alibaba_30s\\'
    },
    {
        'name': 'Google_5m',
        'file': 'Google_cpu_util_aggregated_5m.csv',
        'results_dir': 'h:\\work\\Ablation\\CPU\\SWT-CLSTM-SSC\\cpu_results_Google_5m\\'
    }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\Ablation\\CPU\\SWT-CLSTM-SSC\\CPU_clstm_unified_results\\'
...
def evaluate_model(model, X_test, y_test, scaler, results_dir):
    ...
    np.save(os.path.join(unified_results_dir, f'CLSTM_SSC_{dataset_name}_step_rmse.npy'), np.array(step_rmse))
...
def process_dataset(dataset_info):
    ...
    # 分割数据为训练集和测试集 - 直接使用原始数据，不进行SG滤波和小波变换
...
if __name__ == "__main__":
    ...
    # 保存所有数据集的汇总结果
    with open('h:\\work\\Ablation\\CPU\\SWT-CLSTM-SSC\\CLSTM-SSC_all_datasets_results.json', 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4)

    with open(os.path.join(unified_results_dir, 'clstm-ssc_all_datasets_results.json'), 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4)
