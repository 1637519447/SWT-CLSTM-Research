...
# 支持多个时间间隔数据集的训练与评估
# 移除了平稳小波变换(SWT)和对比学习模块，其他模块保持不变
...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\Ablation\\Mem\\SWT-CLSTM-SC\\mem_results_Alibaba_30s\\'
    },
    {
        'name': 'Google_5m',
        'file': 'Google_mem_util_aggregated_5m.csv',
        'results_dir': 'h:\\work\\Ablation\\Mem\\SWT-CLSTM-SC\\mem_results_Google_5m\\'
    }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\Ablation\\Mem\\SWT-CLSTM-SC\\Mem_clstm_unified_results\\'
...
def evaluate_model(model, X_test, y_test, scaler, results_dir):
    ...
    np.save(os.path.join(unified_results_dir, f'CLSTM_SC_{dataset_name}_test_step_rmse.npy'), np.array(step_rmse))
...
if __name__ == "__main__":
    ...
    # 保存所有数据集的汇总结果
    with open('h:\\work\\Ablation\\Mem\\SWT-CLSTM-SC\\CLSTM-SC_all_datasets_results.json', 'w') as f:
        json.dump(all_datasets_results, f, indent=4)
    
    with open(os.path.join(unified_results_dir, 'clstm_sc_all_datasets_results.json'), 'w') as f:
        json.dump(all_datasets_results, f, indent=4)
