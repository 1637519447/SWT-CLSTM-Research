# 本体 CLSTM-S (PyTorch版本) - 消融实验版本
# 支持多个时间间隔数据集的训练与评估
# 移除了SWT模块，保留SG滤波器，其他模块保持不变
# 移除了所有绘图功能，仅保留数据文件
...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\Ablation\\CPU\\SWT-CLSTM-S\\cpu_alibaba_results_30s\\'
    },
    {
        'name': 'Google_5m',
        'file': 'Google_cpu_util_aggregated_5m.csv',
        'results_dir': 'h:\\work\\Ablation\\CPU\\SWT-CLSTM-S\\cpu_google_results_5m\\'
    }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\Ablation\\CPU\\SWT-CLSTM-S\\CPU_clstm_unified_results\\'
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs, device, results_dir=''):
    ...
    for epoch in range(epochs):
        ...
        print(
            f"Epoch {epoch + 1}/{epochs} - Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}, Time: {epoch_time_ms:.2f} ms")
    ...
    with open(os.path.join(results_dir, f'train_time.json'), 'w', encoding='utf-8') as f:
...
def evaluate_model(model, X_test, y_test, scaler, results_dir):
    ...
    with open(os.path.join(results_dir, f'metrics.json'), 'w', encoding='utf-8') as f:
...
def process_dataset(dataset_info):
    ...
    if window_length < 3:
        ...
        print(f"应用SG滤波器，窗口长度: {window_length}")
    ...
    # 保存原始数据和处理后的数据样本
    np.save(os.path.join(results_dir, 'original_data_sample.npy'), data[:min(1000, len(data))])
    np.save(os.path.join(results_dir, 'smoothed_data_sample.npy'), smoothed_data[:min(1000, len(smoothed_data))])
    ...
    model_complexity_info = {
        ...
        'macs': float(metrics.get('macs', 0)),
        'macs_readable': metrics.get('macs_readable', '0'),
        'params': float(metrics.get('params', 0)),
        'params_readable': metrics.get('params_readable', '0'),
        'total_train_time_ms': float(train_time_info.get('total_train_time_ms', 0)),
        'total_train_time_s': float(train_time_info.get('total_train_time_ms', 0) / 1000),
        'total_train_time_min': float(train_time_info.get('total_train_time_ms', 0) / (1000 * 60)),
        'prediction_time_ms': float(metrics.get('prediction_time_ms', 0)),
        'per_sample_time_ms': float(metrics.get('per_sample_time_ms', 0))
    }
    ...
    # 同时保存到统一结果目录
    with open(os.path.join(unified_results_dir, f'CLSTM-S_{dataset_name}_model_complexity_info.json'), 'w', encoding='utf-8') as f:
        json.dump(model_complexity_info, f, indent=4)
    
    # 保存到统一结果目录
    np.save(os.path.join(unified_results_dir, f'CLSTM-S_{dataset_name}_predictions.npy'), y_pred)
    np.save(os.path.join(unified_results_dir, f'CLSTM-S_{dataset_name}_ground_truth.npy'), y_test_original)
    np.save(os.path.join(unified_results_dir, f'CLSTM-S_{dataset_name}_step_rmse.npy'), np.array(metrics['step_rmse']))
...
if __name__ == "__main__":
    ...
    # 保存所有数据集的汇总结果
    with open('h:\\work\\Ablation\\CPU\\SWT-CLSTM-S\\CLSTM-S_all_datasets_results.json', 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4)

    with open(os.path.join(unified_results_dir, 'clstm-s_all_datasets_results.json'), 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4)
        
    # 保存所有数据集的复杂度信息汇总
    with open('h:\\work\\Ablation\\CPU\\SWT-CLSTM-S\\CLSTM-S_all_complexity_info.json', 'w', encoding='utf-8') as f:
        json.dump(all_complexity_info, f, indent=4)
        
    with open(os.path.join(unified_results_dir, 'clstm-s_all_complexity_info.json'), 'w', encoding='utf-8') as f:
        json.dump(all_complexity_info, f, indent=4)

    print("\n所有数据集处理完成！")
    print(f"结果已保存到各自的目录中，汇总结果保存在: {unified_results_dir}")
    print(f"模型复杂度信息已保存到: {unified_results_dir}/clstm-s_all_complexity_info.json")