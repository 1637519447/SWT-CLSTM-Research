...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\Ablation\\CPU\\SWT-CLSTM-C\\cpu_alibaba_results_30s\\'
    },
    {
        'name': 'Google_5m',
        'file': 'Google_cpu_util_aggregated_5m.csv',
        'results_dir': 'h:\\work\\Ablation\\CPU\\SWT-CLSTM-C\\cpu_google_results_5m\\'
    }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\Ablation\\CPU\\SWT-CLSTM-C\\CPU_swdclstm_unified_results\\'
...
def process_dataset(dataset_info):
    ...
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_C_{dataset_name}_reconstructed_test_ground_truth.npy'),
            reconstructed_truth)
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_C_{dataset_name}_reconstructed_test_step_rmse.npy'),
    ...
    # 同时保存到统一结果目录
    with open(os.path.join(unified_results_dir, f'SWT_CLSTM_C_{dataset_name}_model_complexity_info.json'), 'w', encoding='utf-8') as f:
        json.dump(model_complexity_info, f, indent=4)
...
if __name__ == "__main__":
    ...
    # 保存所有数据集的汇总结果
    with open('h:\\work\\Ablation\\CPU\\SWT-CLSTM-C\\SWT-CLSTM-C_all_datasets_results.json', 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4)

    with open(os.path.join(unified_results_dir, 'swt-clstm-c_all_datasets_results.json'), 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4)
        
    # 保存所有数据集的复杂度信息汇总
    with open('h:\\work\\Ablation\\CPU\\SWT-CLSTM-C\\SWT-CLSTM-C_all_complexity_info.json', 'w', encoding='utf-8') as f:
        json.dump(all_complexity_info, f, indent=4)
        
    with open(os.path.join(unified_results_dir, 'swt-clstm-c_all_complexity_info.json'), 'w', encoding='utf-8') as f:
        json.dump(all_complexity_info, f, indent=4)

    print("\n所有数据集处理完成！")
    print(f"结果已保存到各自的目录中，汇总结果保存在: {unified_results_dir}")
    print(f"模型复杂度信息已保存到: {unified_results_dir}/swt-clstm-c_all_complexity_info.json")