...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\alibaba_mem_results_30s\\'
    },
    {
        'name': 'Google_5m',
        'file': 'Google_mem_util_aggregated_5m.csv',
        'results_dir': 'h:\\work\\google_mem_results_5m\\'
    }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\Mem_swdclstm_unified_results\\'
...
def evaluate_model(model, X_test, y_test, scaler, results_dir, coeff_type='ca'):
    ...
    step_rmse = calculate_step_rmse(y_test_original, y_pred, max_steps)  # 修复：使用正确的变量名
    ...
    metrics = {
        ...
        'step_rmse': [float(x) for x in step_rmse]  # 修复：使用正确的变量名
    }
