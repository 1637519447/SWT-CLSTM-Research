...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\CPU_patchtst_alibaba_results_30s\\'
    },
    {
        'name': 'Google_5m',
        'file': 'Google_cpu_util_aggregated_5m.csv',
        'results_dir': 'h:\\work\\CPU_patchtst_google_results_5m\\'
    }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\CPU_patchtst_unified_results\\'
...
def process_dataset(dataset_info):
    ...
    # 保存模型
    model_path = os.path.join(results_dir, f'patchtst_model_cpu_{dataset_name}.pt')
    ...
    if dataset_name == '30s':
        plt.xlabel('预测步长 (秒)')
        plt.title('PatchTST 30秒级预测的RMSE随预测步长的变化')
    elif dataset_name == '5m':
        plt.xlabel('预测步长 (分钟)')
        plt.title('PatchTST 5分钟级预测的RMSE随预测步长的变化')
    elif dataset_name == '1h':
        plt.xlabel('预测步长 (小时)')
        plt.title('PatchTST 1小时级预测的RMSE随预测步长的变化')
    ...
    # 同时保存到统一目录
    unified_rmse_plot_path = os.path.join(unified_results_dir, f'PatchTST_{dataset_name}_step_rmse_plot.png')
    ...
    plt.legend()
    plt.title(f'Train Set Predictions vs Ground Truth - PatchTST - {dataset_name}')
    ...
    plt.legend()
    plt.title(f'Test Set Predictions vs Ground Truth - PatchTST - {dataset_name}')
    ...
    print(f"模型复杂度 - MACs: {macs_str}, 参数量: {params_str}")
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        try:
            ...
            all_datasets_results[dataset['name']] = result
        except Exception as e:
            print(f"处理数据集 {dataset['name']} 时出错: {str(e)}")
            continue
    
    # 保存所有数据集的结果
    with open(os.path.join(unified_results_dir, 'patchtst_all_datasets_results.json'), 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4, ensure_ascii=False)
    ...
    for dataset_name, metrics in all_datasets_results.items():
        print(f"\n{dataset_name}:")
        print(f"  测试集RMSE: {metrics['test']['rmse']:.4f}")
        print(f"  测试集MAE: {metrics['test']['mae']:.4f}")
        print(f"  测试集R2: {metrics['test']['r2']:.4f}")
        print(f"  测试集MAPE: {metrics['test']['mape']:.2f}%")
        print(f"  训练时间: {metrics['time']['train_time_ms']:.2f} ms")
        print(f"  模型复杂度: {metrics['complexity']['params_readable']} 参数, {metrics['complexity']['macs_readable']} MACs")