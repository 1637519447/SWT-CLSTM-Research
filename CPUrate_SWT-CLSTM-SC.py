# 本体 CLSTM-SC (PyTorch版本) - 消融实验版本
# 支持多个时间间隔数据集的训练与评估
# 移除了小波变换(SWT)和对比学习模块，其他模块保持不变
# 移除了所有绘图功能，仅保留数据文件
...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\Ablation\\CPU\\SWT-CLSTM-SC\\cpu_alibaba_results_30s\\'
    },
    {
        'name': 'Google_5m',
        'file': 'Google_cpu_util_aggregated_5m.csv',
        'results_dir': 'h:\\work\\Ablation\\CPU\\SWT-CLSTM-SC\\cpu_google_results_5m\\'
    }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\Ablation\\CPU\\SWT-CLSTM-SC\\CPU_clstm_unified_results\\'
...
class CNNLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.fc(features)  # 只返回预测输出，不返回特征
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs, device, results_dir=''):
    ...
    with open(os.path.join(results_dir, 'train_time.json'), 'w', encoding='utf-8') as f:
...
def evaluate_model(model, X_test, y_test, scaler, results_dir):
    ...
    np.save(os.path.join(unified_results_dir, f'CLSTM_SC_{dataset_name}_predictions.npy'), y_pred)
    np.save(os.path.join(unified_results_dir, f'CLSTM_SC_{dataset_name}_ground_truth.npy'), y_test_original)
    np.save(os.path.join(unified_results_dir, f'CLSTM_SC_{dataset_name}_step_rmse.npy'), np.array(step_rmse))
...
if __name__ == "__main__":
    ...
    # 保存所有数据集的汇总结果
    with open('h:\\work\\Ablation\\CPU\\SWT-CLSTM-SC\\CLSTM-SC_all_datasets_results.json', 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4)

    with open(os.path.join(unified_results_dir, 'clstm-sc_all_datasets_results.json'), 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4)
