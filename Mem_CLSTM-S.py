...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\Ablation\\Mem\\SWT-CLSTM-S\\mem_results_Alibaba_30s\\'
    },
    {
        'name': 'Google_5m',
        'file': 'Google_mem_util_aggregated_5m.csv',
        'results_dir': 'h:\\work\\Ablation\\Mem\\SWT-CLSTM-S\\mem_results_Google_5m\\'
    }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\Ablation\\Mem\\SWT-CLSTM-S\\Mem_clstm_unified_results\\'
...
def evaluate_model(model, X_test, y_test, scaler, results_dir):
    ...
    np.save(os.path.join(unified_results_dir, f'CLSTM_S_{dataset_name}_predictions.npy'), y_pred)
    np.save(os.path.join(unified_results_dir, f'CLSTM_S_{dataset_name}_ground_truth.npy'), y_test_original)
    np.save(os.path.join(unified_results_dir, f'CLSTM_S_{dataset_name}_test_step_rmse.npy'), np.array(step_rmse))
...
if __name__ == "__main__":
    ...
    # 保存所有数据集的汇总结果
    with open('h:\\work\\Ablation\\Mem\\SWT-CLSTM-S\\CLSTM-S_all_datasets_results.json', 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4)
    
    with open(os.path.join(unified_results_dir, 'clstm_s_all_datasets_results.json'), 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4)
