# 本体 SG-CNN-SWT-LSTM (PyTorch版本) - 改进版
# 支持多个时间间隔数据集的训练与评估
...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\alibaba_cpu_results_30s\\'
    },
    {
        'name': 'Google_5m',
        'file': 'Google_cpu_util_aggregated_5m.csv',
        'results_dir': 'h:\\work\\google_cpu_results_5m\\'
    }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\CPU_swdclstm_unified_results\\'
...
def generate_augmented_samples(x, augmentation_strength=1, augmentation_type='general'):
    ...
    if augmentation_type == 'low_freq':
        # 进一步降低噪声强度，更好地保持ca系数的趋势特性
        sigma_value = 0.015 * x.std().item()  # 从0.02降低到0.015
        noise = torch.normal(mean=0.0, std=sigma_value, size=x.shape).to(x.device)
        
        # 减少趋势偏移的幅度，避免破坏ca的长期趋势
        ...
        # 高频分量使用更适合的噪声分布，保留尖峰特性
        beta_value = 0.25 * x.abs().max().item()  # 从0.3降低到0.25
        noise = torch.FloatTensor(x.shape).uniform_(-beta_value, beta_value).to(x.device)
        
        # 减少突变概率，避免过度扰动
        spike_prob = 0.03  # 从5%降低到3%
        spikes = (torch.rand(batch_size, seq_len, 1) < spike_prob).float() * 0.08
        spikes = spikes.to(x.device)
        noise = noise + spikes
    else:
        # 一般情况下的噪声生成 - 进一步降低基础噪声强度
        noise_strength = augmentation_strength * 0.3  # 从0.5降低到0.3
        noise = torch.randn_like(x) * noise_strength
    
    # 提高原始数据保真度，减少噪声影响
    augmented = x * 0.95 + x * 0.05 * torch.tanh(noise)  # 从0.9/0.1调整到0.95/0.05
    
    # 进一步减小掩码长度，保留更多原始信息
    mask_length = max(1, int(seq_len * 0.03))  # 从5%减少到3%
    ...
    # 降低时间扭曲的频率和幅度
    if augmentation_type == 'low_freq' and torch.rand(1).item() > 0.8:  # 从0.7提高到0.8
        # 低频数据使用更轻微的时间扭曲
        for b in range(batch_size):
            # 生成更平滑、幅度更小的扭曲因子
            warp_factors = torch.sin(torch.linspace(0, 2.0, seq_len)) * 0.02 + 1  # 从0.03降低到0.02
            # 应用扭曲
            augmented[b] = augmented[b] * warp_factors.unsqueeze(-1).to(x.device)
    
    return augmented

def contrastive_loss(features, augmented_features, temperature=10):  # 从8调整到10
...
# 训练函数
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs, device, coeff_type='ca', results_dir=''):
    model.to(device)
    ...
    # 根据系数类型设置对比学习权重和增强类型 - 进一步优化ca
    if coeff_type == 'ca':
        contrastive_weight = 0.012  # 从0.008增加到0.012
        augmentation_type = 'low_freq'
        # ca系数使用更保守的学习率调度
        scheduler_patience = 8  # ca使用更长的patience
    else:
    ...
    for epoch in range(epochs):
        ...
        for inputs, targets in train_pbar:
            ...
            contr_loss = contrastive_loss(features, augmented_features)  # 使用优化后的温度参数
        ...
        if avg_val_loss < best_val_loss:
            ...
            if patience_counter >= patience:
                print(f"早停在第 {epoch+1} 轮 - {coeff_type}")
                break
    
    # 加载最佳模型
    model.load_state_dict(torch.load(os.path.join(results_dir, f'best_model_{coeff_type}.pt')))
    ...
    with open(os.path.join(results_dir, f'train_time_{coeff_type}.json'), 'w') as f:
...
def evaluate_model(model, X_test, y_test, scaler, results_dir, coeff_type='ca'):
    ...
    with open(os.path.join(results_dir, f'metrics_{coeff_type}.json'), 'w') as f:
...
def process_dataset(dataset_info):
    ...
    np.save(os.path.join(unified_results_dir, f'SWD_CLSTM_{dataset_name}_reconstructed_test.npy'), reconstructed_pred)
    np.save(os.path.join(unified_results_dir, f'SWD_CLSTM_{dataset_name}_reconstructed_test_ground_truth.npy'), reconstructed_truth)
    np.save(os.path.join(unified_results_dir, f'SWD_CLSTM_{dataset_name}_reconstructed_test_step_rmse.npy'), np.array(recon_step_rmse))
    ...
    # 保存重构结果
    with open(os.path.join(results_dir, 'reconstructed_metrics.json'), 'w') as f:
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        ...
        # 从结果中提取复杂度信息
        try:
            # 从ca系数的metrics中提取复杂度信息
            ca_metrics = results['ca']
            complexity_info = {
                'macs': ca_metrics['macs'],
                'macs_readable': ca_metrics['macs_readable'],
                'params': ca_metrics['params'],
                'params_readable': ca_metrics['params_readable']
            }
            all_complexity_info[dataset['name']] = complexity_info
        except:
            print(f"无法提取 {dataset['name']} 的复杂度信息")
    
    # 保存所有数据集的汇总结果
    with open('h:\\work\\all_datasets_results.json', 'w') as f:
        json.dump(all_datasets_results, f, indent=4)
    
    with open(os.path.join(unified_results_dir, 'swdclstm_all_datasets_results.json'), 'w') as f:
        json.dump(all_datasets_results, f, indent=4)
    
    # 保存复杂度信息
    with open(os.path.join(unified_results_dir, 'swdclstm_all_complexity_info.json'), 'w') as f:
        json.dump(all_complexity_info, f, indent=4)
    
    print("\n所有数据集处理完成！")
    print(f"结果已保存到各自的目录中，汇总结果保存在: {unified_results_dir}")
    print(f"模型复杂度信息已保存到: {unified_results_dir}/swdclstm_all_complexity_info.json")