...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\Ablation\\Mem\\SWT-CLSTM-SG\\mem_results_Alibaba_30s\\'
    },
    {
        'name': 'Google_5m',
        'file': 'Google_mem_util_aggregated_5m.csv',
        'results_dir': 'h:\\work\\Ablation\\Mem\\SWT-CLSTM-SG\\mem_results_Google_5m\\'
    }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\Ablation\\Mem\\SWT-CLSTM-SG\\Mem_swdclstm_unified_results\\'
...
# 数据增强和对比学习相关函数
def generate_augmented_samples(x, augmentation_strength=0.7, augmentation_type='general'):
    batch_size, seq_len, features = x.shape
    
    # 根据频段类型调整增强策略
    if augmentation_type == 'low_freq':
        noise_strength = augmentation_strength * 0.005
...
def contrastive_loss(features, augmented_features, temperature=4):
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs, device, coeff_type='ca', results_dir=''):
    ...
    # 根据系数类型设置对比学习权重和增强类型
    contrastive_weight = 0.005 if coeff_type == 'ca' else 0.005
...
def evaluate_model(model, X_test, y_test, scaler, results_dir, coeff_type='ca'):
    ...
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_SG_{dataset_name}_predictions_{coeff_type}.npy'), y_pred)
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_SG_{dataset_name}_ground_truth_{coeff_type}.npy'), y_test_original)
...
def process_dataset(dataset_info):
    ...
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_SG_{dataset_name}_reconstructed_test.npy'), reconstructed_pred)
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_SG_{dataset_name}_reconstructed_test_ground_truth.npy'), reconstructed_truth)
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_SG_{dataset_name}_reconstructed_test_step_rmse.npy'), np.array(recon_step_rmse))
...
if __name__ == "__main__":
    ...
    # 保存所有数据集的汇总结果
    with open('h:\\work\\Ablation\\Mem\\SWT-CLSTM-SG\\SWT-CLSTM-SG_all_datasets_results.json', 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4)
    
    with open(os.path.join(unified_results_dir, 'swt_clstm_sg_all_datasets_results.json'), 'w', encoding='utf-8') as f:
        json.dump(all_datasets_results, f, indent=4)
