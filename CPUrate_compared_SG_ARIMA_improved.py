...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\alibaba_cpu_arima_results_30s\\'
    },
    {
        'name': 'Google_5m',
        'file': 'Google_cpu_util_aggregated_5m.csv',
        'results_dir': 'h:\\work\\google_cpu_arima_results_5m\\'
    }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\CPU_arima_unified_results\\'
...
if __name__ == "__main__":
    ...
    # 保存模型复杂度汇总
    with open('h:\\work\\arima_model_complexity.json', 'w') as f:
    ...
    # 移除绘制所有数据集的RMSE随预测步长变化的对比图的代码

    print("\n所有数据集处理完成！")
    print(f"结果已保存到各自的目录中，汇总结果保存在: {unified_results_dir}")
    print(f"模型复杂度汇总保存在: h:\\work\\arima_model_complexity.json")