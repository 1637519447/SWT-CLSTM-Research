...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\alibaba_mem_tfc_results_30s\\'
    },
    # {
    #     'name': 'Google_5m',
    #     'file': 'Google_mem_util_aggregated_5m.csv',
    #     'results_dir': 'h:\\work\\google_mem_tfc_results_5m\\'
    # }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\Mem_tfc_unified_results\\'
...
class TimeEncoder(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.conv1 = nn.Conv1d(input_dim, hidden_dim // 4, kernel_size=11, padding=5, dilation=1)
        self.conv2 = nn.Conv1d(hidden_dim // 4, hidden_dim // 2, kernel_size=11, padding=5, dilation=1)
        self.conv3 = nn.Conv1d(hidden_dim // 2, output_dim, kernel_size=11, padding=5, dilation=1)
...
class TFCModel(nn.Module):
    def __init__(self, input_dim=60, hidden_dim=128, projection_dim=32,
                 output_dim=1):  # 增加hidden_dim到384，projection_dim到64
...
def process_dataset(dataset_info):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_X, batch_y in tqdm(train_loader, desc=f'Epoch {epoch+1}/{num_epochs}'):
            ...
            # 减少L1正则化强度
            l1_lambda = 0.0005  # 从0.001降低到0.0005
            ...
            # 适度放宽梯度裁剪
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)  # 从0.5增加到1.0
            optimizer.step()
        ...
        if (epoch + 1) % 5 == 0:
            print(f'Epoch [{epoch+1}/{num_epochs}], Train Loss: {avg_loss:.6f}, '
                  f'Contrast Loss: {avg_contrast_loss:.6f}, Val Loss: {avg_val_loss:.6f}, '
                  f'LR: {current_lr:.6f}, Contrast Weight: {contrast_weight:.3f}')
        ...
        if avg_val_loss < best_loss:
            ...
            torch.save({
                ...
                'epoch': epoch,
                'best_loss': best_loss
            }, os.path.join(dataset_info['results_dir'], 'tfc_model.pt'))
        else:
            patience_counter += 1
            if patience_counter >= patience:
                print(f"早停在第 {epoch+1} 轮，最佳验证损失: {best_loss:.6f}")
                break
    ...
    # 加载最佳模型
    checkpoint = torch.load(os.path.join(dataset_info['results_dir'], 'tfc_model.pt'))
    ...
    with torch.no_grad():
        # 训练集预测
        for batch_X, batch_y in DataLoader(train_dataset, batch_size=16, shuffle=False):
            predictions = model(batch_X)
    ...
    train_targets_rescaled = scaler.inverse_transform(train_targets.reshape(-1, 1)).flatten()
    test_predictions_rescaled = scaler.inverse_transform(test_predictions.reshape(-1, 1)).flatten()
    test_targets_rescaled = scaler.inverse_transform(test_targets.reshape(-1, 1)).flatten()
