# 本体 SWT-CLSTM-C (PyTorch版本) - 消融实验版本
# 支持多个时间间隔数据集的训练与评估
# 移除了对比学习模块，其他模块保持不变
...
datasets = [
    {
        ...
        'results_dir': 'h:\\work\\Ablation\\Mem\\SWT-CLSTM-C\\mem_results_Alibaba_30s\\'
    },
    {
        'name': 'Google_5m',
        'file': 'Google_mem_util_aggregated_5m.csv',
        'results_dir': 'h:\\work\\Ablation\\Mem\\SWT-CLSTM-C\\mem_results_Google_5m\\'
    }
]

# 创建统一的结果目录
unified_results_dir = 'h:\\work\\Ablation\\Mem\\SWT-CLSTM-C\\Mem_swdclstm_unified_results\\'
...
def evaluate_model(model, X_test, y_test, scaler, results_dir, coeff_type='ca'):
    ...
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_C_{dataset_name}_{coeff_type}_predictions.npy'), y_pred)
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_C_{dataset_name}_{coeff_type}_ground_truth.npy'), y_test_original)
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_C_{dataset_name}_{coeff_type}_test_step_rmse.npy'), np.array(step_rmse))
...
def process_dataset(dataset_info):
    ...
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_C_{dataset_name}_reconstructed_test.npy'), reconstructed_pred)
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_C_{dataset_name}_reconstructed_test_ground_truth.npy'), reconstructed_truth)
    np.save(os.path.join(unified_results_dir, f'SWT_CLSTM_C_{dataset_name}_reconstructed_test_step_rmse.npy'), np.array(recon_step_rmse))
...
if __name__ == "__main__":
    ...
    # 保存所有数据集的汇总结果
    with open('h:\\work\\Ablation\\Mem\\SWT-CLSTM-C\\SWT-CLSTM-C_all_datasets_results.json', 'w') as f:
        json.dump(all_datasets_results, f, indent=4)
    
    with open(os.path.join(unified_results_dir, 'swt_clstm_c_all_datasets_results.json'), 'w') as f:
        json.dump(all_datasets_results, f, indent=4)
